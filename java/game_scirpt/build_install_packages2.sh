...
#添加到本地服务器列表
mysql -e "INSERT INTO serverlist.serverlist2 VALUES (${serverid},'${toGameIp}','${toDbIp}','${serverid}服')"
...
#DB和GAME远程安装JDK
echo "======================start login ${toDbIp} gameserver install jdk==============="
scp -P36000 -r /root/installpackage/jdk1.6.0_23.tar.gz root@${toDbIp}:/tmp/
ssh -p36000 ${toDbIp} "/root/gamescript/install_jdk.sh" 
...
echo "======================install jdk end=========================================="
ssh -p36000 ${toGameIp} "sh /root/gamescript/ulimit.sh"
...
#启动游戏服务器
ssh -p36000 10.142.8.68 "mysql -e \"INSERT INTO gamedata.across_game_server VALUES ('${serverid}','${toGameIp}',9528);\";"
ssh -p36000 10.142.8.68 "mysql -e \"update gamedata.dbcachelist set version=version+1 where  cachename='across_game_server'\";"
echo "脚本执行完毕!!!"
